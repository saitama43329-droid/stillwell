    ...
    <main className="min-h-screen bg-cream">
      ...
      <header className="px-6 py-5 md:px-12 lg:px-16 bg-warmWhite/95 backdrop-blur-sm sticky top-0 z-50 border-b border-charcoal/10">
        <nav className="max-w-7xl mx-auto flex justify-between items-center">
          <Link href="/" className="text-2xl md:text-3xl font-serif font-bold text-charcoal hover:text-sage transition-colors">StillWell</Link>
          <div className="hidden md:flex items-center gap-6 text-sm font-medium text-charcoal">
            <Link href="/" className="hover:text-sage transition-all duration-200">{nav.home}</Link>
            <Link href="/about" className="hover:text-sage transition-all duration-200">{nav.about}</Link>
            <Link href="/lessons" className="hover:text-sage transition-all duration-200">{nav.lessons}</Link>
            <Link href="/start-journey" className="hover:text-sage transition-all duration-200">{nav.contact}</Link>
            <LanguageSwitcher />
          </div>
          <div className="flex md:hidden items-center gap-4">
            <LanguageSwitcher />
            <button onClick={() => document.getElementById('mobile-menu')?.classList.toggle('translate-x-full')} className="p-2 text-charcoal hover:text-sage transition-colors"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg></button>
          </div>
        </nav>
        <div id="mobile-menu" className="md:hidden fixed top-0 right-0 h-full w-72 shadow-2xl transform translate-x-full transition-transform duration-300 ease-in-out z-50" style={{ backgroundColor: '#ffffff' }}>
          <div className="flex flex-col h-full" style={{ backgroundColor: '#ffffff' }}>
            <div className="flex justify-between items-center p-6 border-b border-charcoal/10" style={{ backgroundColor: '#ffffff' }}>
              ...
              <button onClick={() => document.getElementById('mobile-menu')?.classList.add('translate-x-full')} className="p-2 text-charcoal hover:text-sage transition-colors"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
            </div>
            <div className="flex flex-col gap-2 p-6" style={{ backgroundColor: '#ffffff' }}>
              <Link href="/" className="text-lg font-medium text-charcoal hover:text-sage hover:bg-sage/10 px-4 py-3 rounded-lg transition-all" onClick={() => document.getElementById('mobile-menu')?.classList.add('translate-x-full')}>{nav.home}</Link>
              <Link href="/about" className="text-lg font-medium text-charcoal hover:text-sage hover:bg-sage/10 px-4 py-3 rounded-lg transition-all" onClick={() => document.getElementById('mobile-menu')?.classList.add('translate-x-full')}>{nav.about}</Link>
              <Link href="/lessons" className="text-lg font-medium text-charcoal hover:text-sage hover:bg-sage/10 px-4 py-3 rounded-lg transition-all" onClick={() => document.getElementById('mobile-menu')?.classList.add('translate-x-full')}>{nav.lessons}</Link>
              <Link href="/start-journey" className="text-lg font-medium text-charcoal hover:text-sage hover:bg-sage/10 px-4 py-3 rounded-lg transition-all" onClick={() => document.getElementById('mobile-menu')?.classList.add('translate-x-full')}>{nav.contact}</Link>
            </div>
          </div>
        </div>
      </header>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {lessonHighlights.map((item, idx) => (
              <div
                key={idx}
                className="bg-warmWhite p-8 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1"
              >
      ...
      <section className="px-6 py-20 bg-warmWhite">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-4xl font-serif font-bold text-center mb-4 text-charcoal">
            {t.optionsTitle}
          </h2>
          <p className="text-center text-charcoal/70 mb-12 text-lg">
            {t.optionsSubtitle}
          </p>
          <div className="grid md:grid-cols-3 gap-8">
            {pricingOptions.map((option, idx) => (
              <div
                key={idx}
                className={`bg-cream p-8 rounded-3xl shadow-lg border-2 ${
                  idx === 1
                    ? "border-terracotta transform scale-105"
                    : "border-sage/20"
                }`}
              >
                <div className="text-center mb-6">
                  <div className="inline-block px-4 py-1 bg-sage/20 rounded-full text-sm text-sage font-medium mb-4">
                    {option.highlight}
                  </div>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {testimonials.map((testimonial, idx) => (
              <div
                key={idx}
                className="bg-warmWhite p-8 rounded-2xl shadow-lg flex flex-col"
              >
                ...
                <p className="text-charcoal/80 mb-6 italic flex-grow">
                  "{testimonial.text}"
                </p>
      ...
      <section className="px-6 py-20 bg-warmWhite">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="space-y-6">
            <div className="flex gap-6 items-start bg-cream p-6 rounded-2xl">
              <div className="flex-shrink-0 w-14 h-14 bg-sage/20 rounded-full flex items-center justify-center text-sage font-bold text-xl">
                1
              </div>
              <div>
      ...
      <section className="px-4 sm:px-6 py-12 sm:py-16 md:py-20">
        <div className="max-w-3xl mx-auto bg-gradient-to-br from-sage/10 to-terracotta/10 rounded-2xl sm:rounded-3xl p-6 sm:p-8 md:p-12 shadow-2xl text-center space-y-6 sm:space-y-8 border-2 border-sage/20">
          <div className="w-20 h-20 mx-auto bg-warmWhite rounded-full flex items-center justify-center shadow-lg">
          ...
          <div className="h-12 flex items-center justify-center">
            <div className="text-xl md:text-2xl font-serif text-terracotta">
              <span>{ctaText}</span>
          ...
          <a
            ...
            className="group inline-flex items-center gap-3 sm:gap-4 px-8 sm:px-12 py-4 sm:py-6 bg-[#25D366] text-white rounded-full text-base sm:text-lg md:text-xl font-medium hover:bg-[#20BA5A] transition-all shadow-lg hover:shadow-2xl transform hover:-translate-y-1 w-full sm:w-auto justify-center max-w-md mx-auto"
          >
            <svg
